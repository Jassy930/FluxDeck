...
- 创建本地 Gateway
  ...
  - 多 Provider Gateway 已支持有序链路、健康管理与请求级故障切流
  - Anthropic `/v1/messages` 已支持在网络错误、`429`、`5xx` 时顺序切到下一个 Provider
...
- 观察请求日志与基础流量指标
  ...
  - 原生端 `Traffic` 主图区支持按模型堆叠的 token 趋势、图例与 hover tooltip
  - 原生端 `Topology` 页支持三列 Sankey 主舞台，可在链路内按 model 分层查看 token 密度、Top N 高亮、最小流带宽度与 hover 诊断 tooltip
- 安全应用运行时配置
...
- 观察最近流量、错误与请求日志
- 原生端 Logs 工作台现为单列可展开紧凑日志行，便于直接扫描模型映射、错误摘要与 token 细项
